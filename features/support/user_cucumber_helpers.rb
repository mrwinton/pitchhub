module UserCucumberHelpers
  ...
  def confirm_login
    page.has_text?("#{@me.first_name} #{@me.last_name}")
  end
  ...
  def confirm_getting_started_contents
    expect(page).to have_text("Well, hello there!")
    expect(page).to have_text("Who are you?")
    expect(page).to have_text("What are you into?")

    # the username that was just entered for registration
    expect(page).to have_field("first_name", with: @username)
  end
end

World(UserCucumberHelpers)
